...
std::vector<Token> tokenize(const std::string& input) {
    ...
    while (i < input.length()) {
        ...
        switch (input[i]) {
            ...
            case '*': tokens.push_back({TOKEN_STAR, "*"}); break;
            case '/': tokens.push_back({TOKEN_SLASH, "/"}); break;
            default: std::cerr << "Unexpected character: " << input[i] << std::endl; exit(1);
        }
        i++;
    }
...
int evaluate(const std::unique_ptr<ASTNode>& node) {
    ...
    if (auto binOp = dynamic_cast<BinaryOpNode*>(node.get())) {
        ...
        switch (binOp->op) {
            case TOKEN_PLUS: return leftValue + rightValue;
            case TOKEN_MINUS: return leftValue - rightValue;
            case TOKEN_STAR: return leftValue * rightValue;
            case TOKEN_SLASH: return leftValue / rightValue;
            default: std::cerr << "Unknown operator" << std::endl; exit(1);
        }
    }
